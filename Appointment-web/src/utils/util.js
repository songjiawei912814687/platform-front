...
export const formatTimes = (value, type) => {
  ...
  const _date = `${_value.getFullYear()}-${addZero((_value.getMonth() + 1))}-${addZero(_value.getDate())}`;
  const _time = `${addZero(_value.getHours())}:${addZero(_value.getMinutes())}:${addZero(_value.getSeconds())}`;
  if (type == 'date') {
    return _date;
  } if (type == 'time') {
    return _time;
...
export const apiVersion = '/api-api';
