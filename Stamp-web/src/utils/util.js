...
export const transformTree = (data) => {
  if (data && data.length) {
    return data.map(item => ({
      ...item,
      label: item.name ==='' || item.name ===undefined ?item.label:item.name,
...
export const transformTree2 = (data, defaultCheckedKeys, halfCheckedKeys) => {
    if (data && data.length) {
        return data.map(item => {
            if (String(item.checkState) === '1'){
                if(item.children !== undefined && item.children !== 'undefined' && item.children.length > 0){
                    halfCheckedKeys.push(String(item.id) + '_' + String(item.parentId))
                }else{
                    defaultCheckedKeys.push(String(item.id) +  '_' + String(item.parentId));
                }
            }
            return {
                ...item,
                title: item.name,
                value: String(item.id) +  '_' + String(item.parentId),
                key: String(item.id) +  '_' + String(item.parentId),
                children: transformTree2(item.children, defaultCheckedKeys, halfCheckedKeys),
            }
        });
    }
    return [];
...
export const apiVersion = '/api-stamp';
