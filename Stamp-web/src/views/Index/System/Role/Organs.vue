...
<script>
    ...
    import {getPermissions,savePermissions} from '@/remote/roleOrgan'
        ...
        methods: {
            getPermissions(){
                autoDoFn(async () => {
                    ...
                    const res = await getPermissions({roleId: this.roleId});
                    if(res){
                        let checkedKeys = [];
                        let halfCheckedKeys = [];
                        this.permissionsData = transformTree2(res.data, checkedKeys, halfCheckedKeys);
