    ...
    <div class="content-wrap">
        ...
        <search-box @onSearch="handleSearch" @onReset="handleReset">
            ...
            <search-box-item label="状态：">
                <a-select placeholder='请选择状态' v-model="searchParams.state" style="width:100%">
        ...
        <div class="action">
            <a-button @click="handleDrawerToggle('addVisible', true)" v-if="authorityActionList.indexOf('A_SYSTEM_ASSESSMENT_APPRAISALRULE_ADD') > -1 || !useAuthority">录入</a-button>
        </div>
        <div class="content">
            <a-table :columns="columns"
                ...
                <span slot="action" slot-scope="text, record">
                    <a href="javascript:;" @click="handleViewShow(record)" v-if="authorityActionList.indexOf('A_SYSTEM_ASSESSMENT_APPRAISALRULE_VIEW') > -1 || !useAuthority">查看</a>
                    <template v-if="authorityActionList.indexOf('A_SYSTEM_ASSESSMENT_APPRAISALRULE_EDITOR') > -1 || !useAuthority">
                        <a-divider type="vertical"/>
                        <a href="javascript:;" @click="handleModifyShow(record)">编辑</a>
                    </template>
                    <template v-if="record.state === 1 && authorityActionList.indexOf('A_SYSTEM_ASSESSMENT_APPRAISALRULE_START') > -1 || !useAuthority">
                        <a-divider type="vertical"/>
                        <a href="javascript:;" @click="handleChangeState(record,true)">启用</a>
                    </template>
                   <template v-else-if="record.state === 0 && authorityActionList.indexOf('A_SYSTEM_ASSESSMENT_APPRAISALRULE_STOP') > -1 || !useAuthority">
                        <a-divider type="vertical"/>
                        <a href="javascript:;" @click="handleChangeState(record,false)">停用</a>
                    </template>
                    <template v-if="authorityActionList.indexOf('A_SYSTEM_ASSESSMENT_APPRAISALRULE_DELETE') > -1 || !useAuthority">
                        <a-divider type="vertical"/>
