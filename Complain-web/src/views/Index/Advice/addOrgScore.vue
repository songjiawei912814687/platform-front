<template>
    <Drawer :value="addOrgScoreVisible"
        ...
        <div :style="{height: scroll.y + 100 + 'px', overflow: 'auto'}" >
            <a-table :columns="columns"
                ...
                <template slot="input" slot-scope="text, record">
                    <editable-cell v-if="(record.scoreSourceName!='自动计算')" :text="record.quantity" @change="value => onCellChange(record, value)"/>
                    <span v-else>{{record.quantity}}</span>
                </template>
                <template slot="textInput" slot-scope="text, record">
                    <editabletext-cell  v-if="(record.indexId!='32'&&record.indexId!='36')&&(state!=0)" :text="record.ratingDescription" @change="value => onCellTextChange(record, value)"/>
...
<script>
    ...
    import {formOrgScore,descrptionEdit,getApprialItemDetail} from '@/remote/advice';
    import {findDepartmentPlan} from '@/remote/common'
    ...
    export default {
        ...
        props: ['addOrgScoreVisible', 'handleClose', 'organizationId','state', 'year','month','permission', 'staffList', 'orgTreeData', 'roleList'],
        data() {
            const columns = [
                {
                    title: '指标项',
        ...
        watch: {
            addOrgScoreVisible() {
                if (this.addOrgScoreVisible) {
                    autoDoFn(async () => {
                        this.loading = true;
                        const res = await findDepartmentPlan({
                            organizationId:this.organizationId,
                            year:this.year,
                            month:this.month,
                        });
                        if (res.success) {
                            this.depamentPlan = res.data;
                        }
                        const res1 = await getApprialItemDetail({
                            planId:this.depamentPlan[0].id,
                        })
                        if(res1.success){
                            this.data = res1.data;
                            this.groupByData = _.groupBy(res1.data, 'indexId');
                            this.numberList = res1.data.map(item => ({
        ...
        methods: {
            ...
            handleSubmit(e) {
                ...
                this.form.validateFields((err, values) => {
                    if (!err) {
                        autoDoFn(async () => {
                            this.addLoading = true;
                            const res = await formOrgScore(this.modifyId, {
                                ...values,
            ...
            onCellChange(record, value) {
                autoDoFn(async () => {
                    ...
                    const res = await getApprialItemDetail({planId: record.planId});
                    if (res) {
                        this.data = res.data;
            ...
            handleAddSuccess() {
                ...
                //   const res = await getApprialItemDetail({planId: this.modifyId});
                //   if (res) {
                //     this.data = res.data;
                //     this.numberList = res.data.map(item => ({
                //       [String(item.id)]: item.quantity,
