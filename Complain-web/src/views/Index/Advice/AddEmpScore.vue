<template>
    <Drawer :value="addEmpScoreVisible"
            @on-close="handleClose"
            :mask-closable="false"
            :width="650"
            :title="title">
        <a-spin tip="正在加载中..." :spinning="addLoading">
            <a-form v-if="addEmpScoreVisible" @submit="handleSubmit" layout="vertical"
        ...
        <score-staff-modal :membersVisible="selectStaffVisible"
...
import {getOrganizationTree,getRuleByIndexId} from '@/remote/common';
import  {formEmployeeRecordWithNoAppraisal} from '@/remote/advice';
import moment from 'moment';
import ScoreStaffModal from '@/components/StaffModal/ScoreStaffModal';
...
export default {
  ...
  props: ['addEmpScoreVisible', 'handleClose', 'classifyList','ruleList','organizationOptions'],
  data() {
    return {
      addLoading: false,
      ruleList: [],
        ...
        this.form.validateFields((err, values) => {
            if (!err) {
                autoDoFn(async () => {
                    ...
                    const res = await formEmployeeRecordWithNoAppraisal(null,params);
                    if (res.success) {
