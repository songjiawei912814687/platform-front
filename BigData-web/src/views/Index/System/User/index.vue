<template>
    <div class="content-wrap">
        ...
        <search-box @onSearch="handleSearch" @onReset="handleReset">
            ...
            <search-box-item label="用户名：">
                <a-input v-model="searchParams.username" placeholder="请输入用户名"/>
            </search-box-item>
            <search-box-item label="用户类型：">
                <a-select v-model="searchParams.userType" placeholder="请选择用户类型" style="width:100%">
        ...
        <div class="action">
            <a-button @click="handleDrawerToggle('addVisible', true)"
                      v-if="authorityActionList.indexOf('A_SYSTEM_SYSTEMSETTINGS_USERMANAGEMENT_ADD') > -1">录入
            </a-button>
        </div>
        <div class="content">
            <a-table :columns="columns"
                ...
                <span slot="typeUser"
                      slot-scope="text, record">{{String(record.userType) === '0' ? '用户账号' : '部门账号'}}</span>
                <span slot="action" slot-scope="text, record">
                    ...
                    <!--<template v-if="authorityActionList.indexOf('A_SYSTEM_SYSTEMSETTINGS_USERMANAGEMENT_EDITOR') > -1">-->
                    ...
                    <template v-if="authorityActionList.indexOf('A_SYSTEM_SYSTEMSETTINGS_USERMANAGEMENT_DELETE') > -1">
                        <a-divider type="vertical"/>
...
<script>
  ...
  import {getPage, getDel, getByRoleId, saveByRoleId, getUsersDel} from '@/remote/system-user';
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'userType',
      width: 700,
      scopedSlots: {customRender: 'typeUser'},
    },
    {
      title: '操作',
      key: 'action',
