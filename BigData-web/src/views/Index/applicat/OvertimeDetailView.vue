<template>
    <a-drawer :visible="overtimeVisible"
              @close="handleClose"
              :loading="viewLoading"
              :destroyOnClose="true"
              :maskClosable="false"
...
import { getOvertimeDetail } from '@/remote/dtailView';
...
export default {
  ...
  props: ['overtimeVisible', 'handleClose', 'modifyId','resourceId'],
  data() {
    return {
      viewData: {},
      viewLoading: false,
  ...
  watch: {
      ...
      if (this.modifyId) {
        autoDoFn(async () => {
          ...
          const res = await getOvertimeDetail({ id: this.resourceId });
          if (res) {
            this.viewData = {
              ...res.data,
            };
