...
<script>
  import {getDetail,getLeaveDetail} from '@/remote/reportMonth';
      ...
      modal() {
        if (this.modal.visible) {
          autoDoFn(async () => {
            this.loading = true;
            let res;
            if(this.modal.record.type===4){
              res = await getLeaveDetail(this.modal.record);
            }else{
              res = await getDetail(this.modal.record);
            }
