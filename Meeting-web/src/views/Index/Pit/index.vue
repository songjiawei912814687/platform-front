<template>
    <div class="content-wrap">
        <h2 class="content__title">开评标室管理</h2>
        <search-box @onSearch="handleSearch" @onReset="handleReset">
            <search-box-item label="开评标室名称：">
                <a-input v-model="searchParams.name" placeholder="请输入开评标室名称"/>
            </search-box-item>
        </search-box>
        <div class="action">
            <a-button @click="handleDrawerToggle('addVisible', true)" v-if="authorityActionList.indexOf('A_MEETING_PIT_ADD') > -1">录入</a-button>
        </div>
        <div class="content">
            <a-table :columns="columns"
                ...
                <span slot="action" slot-scope="text, record">
                    <a href="javascript:;" @click="handleViewShow(record)" v-if="authorityActionList.indexOf('A_MEETING_PIT_VIEW') > -1">查看</a>
                    <template>
                        <a-divider type="vertical"/>
                        <a href="javascript:;" @click="handleModifyShow(record)" v-if="authorityActionList.indexOf('A_MEETING_PIT_EDITOR') > -1">编辑</a>
                    </template>
                    <template>
                        <a-divider type="vertical"/>
                        <a-popconfirm title="确认要删除此条数据?" @confirm="handleDelete({idList: record.id})" okText="确认" cancelText="取消">
                            <a href="javascript:;" v-if="authorityActionList.indexOf('A_MEETING_PIT_DELETE') > -1">删除</a>
                        </a-popconfirm>
                    </template>
                </span>
            </a-table>
...
import { getPage, getDel, exportExcel } from '@/remote/pit';
