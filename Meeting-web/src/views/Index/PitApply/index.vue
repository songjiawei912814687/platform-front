<template>
	<div class="content-wrap">
		<h2 class="content__title">开平标室预约</h2>
		...
		<div class="room-content__header">
			<div class="room-content-row clearfix">
				...
				<div class="room-content-col" v-for="item in meetingTime1">{{item.name}}</div>
			</div>
		</div>
				...
				<div class="room-content__content">
					<div>
						<div class="room-content-row clearfix" v-for="room in data">
							...
							<div class="room-content-col" v-for="item in meetingTime1">
								<p class="room-appointment" v-if="!isOverDate && authorityActionList.indexOf('A_MEETING_PITAPPLY_ADD') > -1"
								 @click="handleAddShow(item, room)">预约</p>
							</div>
							<div class="room-content-meeting" v-for="meeting in room.pitApplyOutputList" :style="{width: (meeting.endDateTime - meeting.startDateTime + 1) * 5 + '%', left:((meeting.startDateTime + 1) * 5 ) + '%' }">
								<a-popover placement="bottom" :title="meeting.theme">
									<div slot="content" style="max-width: 500px">
										<p>主办单位：{{meeting.hostUnit}}</p>
										<p>会议时间：{{meetingDate + ' ' + meetingTime1[meeting.startDateTime].name}} -
											{{meetingDate + ' ' + meetingTime1[meeting.endDateTime].name}}</p>
										<p>申请人：{{meeting.employeesName}}</p>
										<p v-if="meeting.attachmentList.length">附件：
											<a v-for="file in meeting.attachmentList" :href="file.url" target="_blank">{{file.fileName}}&nbsp;&nbsp;&nbsp;</a>
										</p>
										<div style="text-align: right;margin-top: 10px" v-show="meeting.isCreator" v-if="authorityActionList.indexOf('A_MEETING_PITAPPLY_DELETE') > -1">
											<a-popconfirm v-if="meeting.employeeId === employeesId" title="确认要删除此条数据?" @confirm="handleDelete({idList: meeting.id})"
											 okText="确认" cancelText="取消">
												<a-button type="danger">取消</a-button>
											</a-popconfirm>
											<a-button @click="handleViewShow(meeting)" style="margin-left: 10px">查看
											</a-button>
											<a-button v-if="meeting.state === 0" type="primary" style="margin-left: 10px" @click="handleModifyShow(meeting, room)">修改
											</a-button>
										</div>
									</div>
		...
		<add :addVisible="addVisible" @successCallback="handleAddSuccess" :staffList="staffList" :modifyId="modifyId" :room="room"
		 :meetingDate="meetingDate" :startDateTime="startTime" :handleClose="() => handleDrawerToggle('addVisible', false)"></add>
	...
	export default {
		...
		methods: {
			...
			handleAddShow(item, room) {
				...
				if (item.value) {
					console.log(this.searchParams.date.dayOfYear());
