<template>
	<div class="content-wrap">
		<h2 class="content__title">已发件箱</h2>
			 ...
			 @change="onPageChange" :pagination="pagination">
				<span slot="action" slot-scope="text, record">
					...
					<template v-if="authorityActionList.indexOf('A_ASSESSMENT_APPRAISALRULE_EDITOR') > -1 || !useAuthority">
						...
						<a href="javascript:;" @click="showModal()">回复</a>
					</template>
		...
		<a-modal title="回复列表" v-model="membersVisible" @ok="handleOk" :width="600">
			<a-table :columns="membersColumns" :destroyOnClose="true" :dataSource="membersData" @change="onMembersPageChange"
			 :rowKey="record => String(record.id)" :pagination="membersPagination"></a-table>
		</a-modal>
	...
	export default {
		...
		data() {
			return {
				...
				loading: false,
				scroll: {
					y: document.documentElement.clientHeight - 280,
					x: 1000,
				},
				state: '', // 状态
		...
		methods: {
			...
			initList() {
				autoDoFn(async () => {
					this.loading = true;
					const res = await getPageSuccess({
						...this.searchParams,
						...this.pagination,
			...
			showModal() {
				console.log(this.membersVisible);
