<template>
    <div class="content-wrap">
        <h2 class="content__title">人员岗位变动</h2>
        ...
        <div class="action">
            <a-button @click="handleDrawerToggle('addVisible', true)" v-if="authorityActionList.indexOf('A_PERSONNEL_JOBCHANGE_ADD') > -1">录入</a-button>
        </div>
        <div class="content">
            <a-table :columns="columns"
                ...
                <span slot="action" slot-scope="text, record">
                    <a href="javascript:;" @click="handleViewShow(record)" v-if="authorityActionList.indexOf('A_PERSONNEL_JOBCHANGE_VIEW') > -1">查看</a>
                    <template v-if="authorityActionList.indexOf('A_PERSONNEL_JOBCHANGE_EDITOR') > -1">
                        <a-divider type="vertical"/>
                        <a href="javascript:;" @click="handleModifyShow(record)">编辑</a>
                    </template>
                    <template v-if="authorityActionList.indexOf('A_PERSONNEL_JOBCHANGE_DELETE') > -1">
                        <a-divider type="vertical"/>
...
import { getPage, getDel } from '@/remote/post';
import { getOrgTreeWithOutAuthority, getEmployeesAll, getRoleAll } from '@/remote/base';
...
const columns = [
  ...
  {
    title: '原部门',
    dataIndex: 'originalOrganizationName',
    width: 200,
  },
  {
    title: '原职务',
    dataIndex: 'originalJobName',
    width: 200,
  },
  {
    title: '现部门',
    dataIndex: 'nowOrganizationName',
    width: 200,
  },
  {
    title: '现职务',
    dataIndex: 'nowJobName',
    width: 200,
  },
  {
    title: '时间',
