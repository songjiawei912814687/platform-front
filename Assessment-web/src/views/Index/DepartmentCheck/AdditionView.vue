<template>
    <a-drawer :visible="viewVisible"
        ...
        <div class="drawer-footer">
            ...
            <a-button @click="handleSubmit" type="primary" :loading="addLoading" v-if="state==0||state==1||permission==1" >保存</a-button>
        </div>
    </a-drawer>
</template>
...
import { saveAddtition,getAddition } from '@/remote/departmentCheck';
const { Item } = Form;
    export default {
        name: "detailView",
        components: {
        ...
        props: ['viewVisible', 'handleClose', 'modifyId','permission','state'],
        data() {
            return {
                addLoading:false,
                viewData: {},
            ...
            viewVisible() {
                if (this.modifyId) {
                    autoDoFn(async () => {
                        ...
                        const res = await getAddition({ id: this.modifyId });
                        if (res) {
