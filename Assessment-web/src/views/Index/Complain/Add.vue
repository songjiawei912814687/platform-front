...
import { formEditPost, getDetail, formPost } from '@/remote/complain';
...
export default {
  ...
  computed: {
    title() {
      return (this.modifyId && !this.planId) ? '考核申诉编辑' : '考核申诉录入';
    },
  },
  created() {
  ...
  methods: {
    handleSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
					console.log('fdkfdknfd')
          autoDoFn(async () => {
            this.addLoading = true;
            let res;
            if(this.planId){
							console.log("woowowo333333")
              res = await formPost({
                id: this.planId,
