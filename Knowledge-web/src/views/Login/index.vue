        ...
        <div class="login__main">
            ...
            <a-form @submit="handleLogin" :autoFormCreate="(form)=>{this.form = form}">
                ...
                <a-form-item
                    ...
                    <a-input placeholder='请输入密码' :type="passwordType" @keyup.enter.native="handleLogin"
                             auto-complete="off">
            ...
            if (getParameterByName('autoLogin')) {
                autoDoFn(async () => {
                    ...
                    if (res) {
                        ...
                        const {
                            employeeId, employeesName, jobId, jobName, organizationId, organName, userName, icon,
                            actions, menuOutputs
                        } = res.data;
                        ...
                        localStorage.setItem('organId', organizationId);
                        localStorage.setItem('organName', organName);
                        localStorage.setItem('userName', userName);
                        localStorage.setItem('icon', icon);
                        localStorage.setItem('authorityMenuList', JSON.stringify(menuOutputs ? menuOutputs.map(item => item.code) : []));
                        localStorage.setItem('authorityActionList', JSON.stringify(actions ? Object.keys(actions) : []));
                    }
                    this.loading = false;
                });
            }
