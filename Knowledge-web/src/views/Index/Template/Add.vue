                ...
                <a-row :gutter="24">
                    ...
                    <a-col :span="24">
                        <a-form-item
                            ...
                            <a-select placeholder='请选择考核对象' :disabled="objectType === ''" mode="multiple">
...
export default {
  ...
  methods: {
        ...
        if (!err) {
          autoDoFn(async () => {
            ...
            const res = await formPost({
              id: this.modifyId,
              objectIdList: values.objectList.join(','),
            }, {
              name: values.name,
    ...
    getAppraisalOrganizationNotInUse() {
      autoDoFn(async () => {
        const res = await getAppraisalOrganizationNotInUse({ templateId: this.modifyId });
        if (res) {
    ...
    getRolesNotInUse() {
      autoDoFn(async () => {
        const res = await getRolesNotInUse({ templateId: this.modifyId });
        if (res) {
          this.objectListData = res.data;
        }
      });
